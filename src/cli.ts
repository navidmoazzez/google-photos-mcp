 ...
 *
 * The command IS the tool name. `create_album` runs as `create-album`, and the
...
type Flag = {
  /** The schema key, e.g. `page_token`. */
  key: string;
  /** The long flag, e.g. `--page-token`. */
  flag: string;
  kind: FlagKind;
  required: boolean;
  repeatable: boolean;
...
function kindOf(schema: ZodTypeAny): { kind: FlagKind; choices?: string[]; repeatable: boolean } {
  ...
  switch (typeName) {
    ...
    case "ZodArray": {
      // An array of scalars is repeatable (`--ids a --ids b`). An array of
...
export function parseArgs(argv: string[], flags: Flag[]): Record<string, unknown> {
  ...
  // One bare argument fills the first required flag, so `get-album "abc"`
...
export function exitCodeFor(error: unknown): number {
  const e = error as { status?: number; reason?: string; message?: string };
 ...
 *
 * The package ships two binaries onto the same file. Printing `google-photos-mcp`
 * at someone who typed `google-photos-cli` hands them a command that works but is
 * not the one they have in their fingers.
 */
function binName(): string {
  const name = (process.argv[1] ?? "").split("/").pop() ?? "";
  return name.startsWith("google-photos-cli") ? "google-photos-cli" : "google-photos-mcp";
}
...
export async function runCli(argv: string[]): Promise<number> {
  ...
  if (!spec) {
    ...
    emitError(
      new Error(
        hidden
          ? `${name} is unavailable: this server is running with GOOGLE_PHOTOS_READ_ONLY=1.`
  ...
  try {
    ...
    // One pool, exactly as the server builds it, so `--account` resolves by the
    // same rules and two accounts share one quota tracker.
    const pool = new ClientPool(config);
    const ctx: ToolContext = makeContext(pool, (args as { account?: string }).account, config, guard);
